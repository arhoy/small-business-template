...
export const query = graphql`
  {
    heroImage1: file(relativePath: { eq: "recipes/Whole_Roasted_fish_2.jpg" }) {
      childImageSharp {
    ...
    heroImage2: file(relativePath: { eq: "recipes/Whole_Roasted_fish.jpg" }) {
      childImageSharp {
        fluid(quality: 90, maxWidth: 1000) {
          ...GatsbyImageSharpFluid_tracedSVG
        }
...
const Whole_Roasted_fish_2 = ({ data }) => {
  const settings = {
    dots: true,
    infinite: true,
    speed: 500,
  ...
  return (
    <Layout>
      <Section>
        <Container1200>
          <Container>
            <Title>{recipeData && recipeData[7].title}</Title>
            <Slider {...settings}>
              <SliderContainer2>
...
export default Whole_Roasted_fish_2;
